...
import { Camera, CheckCircle2, Play, ChevronLeft, AlertCircle } from 'lucide-react'

const API_URL = process.env.NEXT_PUBLIC_API_URL ?? 'http://127.0.0.1:8190'

const stageLabels: Record<string, string> = { laser_cnc: 'Corte CNC Laser', bending: 'Quinagem', guillotine: 'Guilhotina' }
...
export default function OperadorOrdemPage() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/v1/orders/${id}`, { headers: authHeader() })
      .then(r => r.json())
      .then(setOrder)
  ...
  async function handleStart() {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setError(err.error ?? 'Erro ao iniciar')
        return
      }
      const updated: Order = await fetch(`${API_URL}/api/v1/orders/${id}`, { headers: authHeader() }).then(r => r.json())
      setOrder(updated)
      setActionMsg('Etapa iniciada com sucesso!')
  ...
  async function handleComplete() {
    ...
    try {
      ...
      if (!r.ok) {
        const err = await r.json()
        setError(err.error ?? 'Erro ao concluir')
        return
      }
      const updated: Order = await fetch(`${API_URL}/api/v1/orders/${id}`, { headers: authHeader() }).then(r => r.json())
      setOrder(updated)
      setActionMsg('Etapa concluída!')
    } finally { setActing(false) }
  }
  ...
  function handlePhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setPhotoUrl(url)
    // Upload para o backend (implementar em próxima iteração)
  }
    ...
    <div className="p-4 space-y-4">
      ...
      {activeStage && (
        <div className={`rounded-xl border p-4 ${
          ...
          <div className="text-base font-semibold text-white">{stageLabels[activeStage.type]}</div>
            ...
            {activeStage.status === 'in_progress' && (
              <>
                <button onClick={() => fileInput.current?.click()}
                  className="flex items-center gap-2 rounded-lg border border-slate-600 px-4 py-2.5 text-sm font-medium text-slate-300 active:bg-slate-700 transition-colors">
                  <Camera className="h-4 w-4" /> Foto
                </button>
                <button onClick={handleComplete} disabled={acting}
                  className="flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2.5 text-sm font-semibold text-white active:bg-green-700 disabled:opacity-60 transition-colors">
          ...
          {photoUrl && (
            <img src={photoUrl} alt="Foto de prova" className="mt-3 rounded-lg w-full max-h-48 object-cover border border-slate-600" />
          )}
        </div>
      )}
          ...
          {order.stages.map(s => (
            <div key={s.id} className="flex items-center gap-2 text-sm">
              ...
              <span className={s.status === 'pending' ? 'text-slate-500' : 'text-white'}>
                {s.stageNumber}. {stageLabels[s.type]}
              </span>
