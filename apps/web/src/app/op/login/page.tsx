      ...
      <div className="w-full max-w-sm">
        <div className="bg-slate-800 rounded-3xl p-7 border border-slate-700">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className={cls.darkLabel}>Empresa (slug)</label>
              <input type="text" value={tenantSlug} onChange={e => setTenantSlug(e.target.value)}
                required placeholder="demo" className={cls.darkInput} />
            </div>
            <div>
              <label className={cls.darkLabel}>Utilizador</label>
              <input type="text" value={username} onChange={e => setUsername(e.target.value)}
                required placeholder="joao.silva" autoComplete="username" className={cls.darkInput} />
            </div>
            <div>
              <label className={cls.darkLabel}>Palavra-passe</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                required autoComplete="current-password" className={cls.darkInput} />
            </div>
            {error && (
              <div className="rounded-xl bg-red-900/30 border border-red-800 px-4 py-3 text-sm text-red-400">{error}</div>
            )}
            <button type="submit" disabled={loading} className={cls.btnYellow + ' w-full py-3 mt-2'}>
